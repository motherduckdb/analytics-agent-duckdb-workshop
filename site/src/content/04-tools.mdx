...
<CopyBlock label="Test directly (no model needed)" text="python3 agent_tools.py" />

<CopyBlock label="Test via agent" text={`python3 agent_tools.py "Revenue by venue for 2025"
python3 agent_tools.py "Top 5 returning customers"
python3 agent_tools.py "What's the profit margin on beverages?"`} />

<Tip>**Direct call:** `revenue_by_venue(2025)` → same numbers every time. **Via agent:** the model picks the right tool automatically.</Tip>
